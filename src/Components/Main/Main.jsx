...
const Main = () => {
    ...
    const ButtonCall = () => {
        ...
        axios.get(pokeUrl)
            .then((response) => {
                ...
                let pokemonFrontImg = sprites.front_shiny;
                let pokemonBackImg = sprites.back_shiny;
    ...
    return (
        ...
        <main className="max-sm:mt-0 mt-[70px] flex justify-center h-full">
            <div className='max-sm:border-0 max-sm:rounded-none max-sm:w-[100%] max-sm:mt-0 max-sm:h-screen mt-[60px] h-[660px] w-[500px] border-solid border-black border-2 rounded-3xl bg-[lightgreen] '>
                <div className='flex'>
                ...
                <div className='flex justify-center'>
                    <div className='mt-[35px] h-[330px] w-[330px] border-solid rounded-bl-[35px] border-black border-2 bg-[red] flex justify-center flex-col'>
                        <div className='flex justify-center gap-5 mt-3'>
                            <div className=' h-3 w-3 rounded-xl bg-[lightgreen] border-solid border-black border-2'></div>
                            <div className=' h-3 w-3 rounded-xl bg-[lightgreen] border-solid border-black border-2'></div>

                        </div>

                        <div className='justify-center items-center flex flex-col  mt-[17px] h-[250px] w-[270px] border-solid border-black border-2 bg-[lightgreen] m-auto'>
                ...
                <div>
                    <input typeof='text' onFocus={(e) => e.target.value = ""} onChange={(e) => setPokemonUrl(e.target.value.toLowerCase())} className='mt-2 text-center h-8 text-lg rounded-bl-xl border-2 border-black rounded-tr-xl bg-[red] placeholder:text-white text-white' placeholder="Pokemon's Name" type="text" />
                    ...
                    <div className='flex'>
                        ...
                        <button onClick={() => ButtonCall()} className='max-sm:ml-[30px] ml-[65px] border-solid h-16 w-16 rounded-[50%] border-black bg-[red] border-2 text-white hover:shadow-md hover:shadow-stone-700 hover:drop-shadow-md '>Press</button>

                        <div className='mt-9 ml-7 h-[75px] w-[120px] border-solid border-black border-2 bg-[red]'>
                            <h3 className='text-base text-left ml-[2px] text-white'>{pokemonInfo ? `Hp: ${pokemonInfo?.hp}` : ""}</h3>
                            <h3 className='text-base text-left ml-[2px] text-white'>{pokemonInfo ? `Atk: ${pokemonInfo?.atk}` : ""}</h3>
                            <h3 className='text-base text-left ml-[2px] text-white'>{pokemonInfo ? `Def: ${pokemonInfo?.def}` : ""}</h3>
                        ...
                        <div className='max-sm:ml-4 flex ml-12 mt-12'>
                            <button onClick={() => changeImg()} className='border-solid h-8 w-14  border-black bg-[red]  rounded-bl-xl rounded-tl-xl text-white flex justify-center hover:border-2 hover:border-black hover:border-solid' >< BiLeftArrow className='m-auto' /></button>
                            <button onClick={() => changeImg()} className='border-solid h-8 w-14  border-black bg-[red]  rounded-br-xl rounded-tr-xl text-white flex justify-center hover:border-2 hover:border-black hover:border-solid ' >< BiRightArrow className='m-auto' /></button>
                        </div>
                    </div>
